...
for (let i = 0; i <= pokemons.length; i++) {
    ...
    card.innerHTML = `
    <p class="font-[700] ml-[30px] text-[24px]  ">${pokemons[i].num}</p>
    <img src="${pokemons[i].img}" alt="img" class=" mx-auto mt-[30px]">
    <span class="w-full h-[2px] bg-gray-500 block"></span>
    
    <p class="font-[700] ml-[30px] text-[24px] mt-5 ">${pokemons[i].name}</p><i class='bx bx-heart  absolute right-4 text-xl top-[51%]'></i>
    <p class="text-xl  ml-[30px] font-[500] "> ${pokemons[i].type}</p>
    <p class="font-[700] ml-[30px] text-[24px] mt-7 inline-block">${pokemons[i].weight}</p>
    <p class="font-[700] ml-[30px] text-[24px] mt-7 inline-block">${pokemons[i].egg}</p>
    `
    wrapper.append(card)
}